...
import { canRevealProof, proofMatches, verifyGameProof } from "@/lib/game-proof";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!canRevealProof(previousProof, attempt)) return NextResponse.json({ error: "The challenge is not complete." }, { status: 409 });
    const song = await getDailySong(date, category);
    if (!song) return NextResponse.json({ error: "No songs configured." }, { status: 503 });
    return NextResponse.json({ proof: rawProof, answer: { artist: song.artist, title: song.title, soundcloudUrl: song.soundcloudUrl, sourceUrl: song.sourceUrl } }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
